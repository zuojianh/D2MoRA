...
class Trainer:
    ...
    def _remove_unused_columns(self, dataset: "datasets.Dataset", description: Optional[str] = None):
        ...
        signature_columns = self._signature_columns + ["task_id"]
    ...
    def _save_best_checkpoint(self, model, trial, metrics=None):
        ...
        output_dir = run_dir + "-best-model"
        if self.state.best_metric is None:
            best_metric_file = output_dir + "/trainer_state.json"
            if os.path.exists(best_metric_file):
                print("best_metric_file:", best_metric_file)
                with open(best_metric_file, 'r', encoding='utf-8') as f:
                    import json
                    data_dict = json.load(f)
                    # 读取某个键，比如 "eval_loss"
                    best_metric_value = data_dict.get("best_metric", None)
                    print(f"Load best metric {best_metric_value:.4f}")
                    self.state.best_metric = best_metric_value
        if self.args.load_best_model_at_end and metrics is not None:
            current_eval_metrics = metrics.get("eval_loss")
            if current_eval_metrics is not None:
                if (
                        self.state.best_metric is None
                        or current_eval_metrics < self.state.best_metric
                ):
                    self.model.save_pretrained(output_dir)  # 👈 你自定义的保存路径
                    print(f"\nNew best eval_metrics {current_eval_metrics:.4f}, best model saved!")
                    if self.state.best_metric is not None:
                        print(f"Last best eval_metrics {self.state.best_metric:.4f}.")
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        self._save_best_checkpoint(model, trial, metrics=metrics)
        # Save model checkpoint
        checkpoint_folder = f"{PREFIX_CHECKPOINT_DIR}-{self.state.global_step}"
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            if self.use_orth_loss:
                loss = self.compute_loss_with_orthogonal(model, inputs)
            else:
    ...
    def compute_loss_with_orthogonal(self, model, inputs, return_outputs=False):
        """
        How the loss is computed by Trainer. By default, all models return the loss in the first element.
        ...
        device = loss.device
        expert_outputs_A = nn.ParameterDict()
        for name, param in model.named_parameters():
            if 'lora_A' in name:
                replace_name = name.replace(".", "-")
                idx = replace_name.find('lora_A')
                result = replace_name[:idx + len('lora_A')]
                if result not in expert_outputs_A:
                    expert_outputs_A[result] = nn.ParameterList([param])
                else:
                    expert_outputs_A[result].append(param)

        expert_outputs_B = nn.ParameterDict()
        for name, param in model.named_parameters():
            if 'lora_B' in name:
                replace_name = name.replace(".", "-")
                idx = replace_name.find('lora_B')
                result = replace_name[:idx + len('lora_B')]
                if result not in expert_outputs_B:
                    expert_outputs_B[result] = nn.ParameterList([param])
                else:
                    expert_outputs_B[result].append(param)

        orth_loss = torch.tensor(0.0, device=device)
        for key, param_list in expert_outputs_A.items():
            norms = [p.view(-1) / (p.norm() + 1e-6) for p in param_list]
            for i in range(len(norms)):
                for j in range(i + 1, len(norms)):
                    cos_sim = torch.dot(norms[i], norms[j])  # 越接近0越正交
                    orth_loss += torch.abs(cos_sim.to(device))

        for key, param_list in expert_outputs_B.items():
            norms = [p.view(-1) / (p.norm() + 1e-6) for p in param_list]
            for i in range(len(norms)):
                for j in range(i + 1, len(norms)):
                    cos_sim = torch.dot(norms[i], norms[j])  # 越接近0越正交
        ...
        # print(expert_outputs_A)
        # print(loss, orth_loss)
        loss = loss + 1e-4 * orth_loss
        return (loss, outputs) if return_outputs else loss
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    with self.compute_loss_context_manager():
                        if self.use_orth_loss:
                            loss, outputs = self.compute_loss_with_orthogonal(model, inputs, return_outputs=True)
                        else:
